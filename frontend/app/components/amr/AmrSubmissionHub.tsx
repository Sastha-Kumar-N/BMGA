...
type FindingRow = {
  ...
  locations?: Array<{ state?: string | null; district?: string | null; city?: string | null; latitude?: number | null; longitude?: number | null }>;
...
const blankFinding = {
  ...
  state: '', district: '', city: '', latitude: '', longitude: '', institution: '', studyDesign: '', sequencingPlatform: '', analysisMethod: '',
...
export default function AmrSubmissionHub({ initialFindingId, initialPublicationId }: { initialFindingId?: string; initialPublicationId?: string }) {
  ...
  const findingPayload = useCallback(() => ({
    ...
    locations: finding.state || finding.district || finding.city || finding.latitude || finding.longitude ? [{ country: 'India', state: finding.state || undefined, district: finding.district || undefined, city: finding.city || undefined, latitude: finding.latitude.trim() ? Number(finding.latitude) : undefined, longitude: finding.longitude.trim() ? Number(finding.longitude) : undefined }] : [],
  ...
  const saveFinding = async (submit = false) => {
    ...
    const latitude = finding.latitude.trim(); const longitude = finding.longitude.trim();
    if (Boolean(latitude) !== Boolean(longitude)) { setNotice({ type: 'error', text: 'Provide both latitude and longitude, or leave both blank.' }); return; }
    if (latitude && (!Number.isFinite(Number(latitude)) || Number(latitude) < -90 || Number(latitude) > 90 || !Number.isFinite(Number(longitude)) || Number(longitude) < -180 || Number(longitude) > 180)) { setNotice({ type: 'error', text: 'Use valid coordinates: latitude -90 to 90 and longitude -180 to 180.' }); return; }
